...
- `paths.ts` — package root, bundled templates, find the tracker root, new item paths
...
- `board.ts` — static HTML board renderer (`renderBoardHtml` pure; `runBoard` writes the file). Cards are drag-and-drop enabled: the page script embeds `evaluateDrop` (same transition/claim/blocked rules as `status.ts` + `update.ts`), moves optimistically, and POSTs `{id, status, blocked_reason?, assignee?}` to the endpoint (assignee is only sent when claiming an unassigned claimable via the in-progress prompt; force is never sent — claim conflicts stay CLI-only) in `<body data-update-endpoint>` (default `/api/update`). On any failure it reverts and shows the equivalent `arggon update` command. The future `board --serve` server must implement that endpoint by shelling through `arggon update` — the browser never writes tracker files directly; a 2xx `{"ok":true}` persists, any error must answer `{"ok":false,"error":{"message"}}`.
...
- `sync-command.ts` — reconcile open PRs back into the tracker (`--check` default CI gate, `--write` fills empty `branch` fields only; never overwrites, never guesses, never auto-done)
