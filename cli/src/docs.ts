...
const DOC_PATH_MAP: Record<string, string> = {
  ...
  "mcp-json": ".mcp.json",
  // OpenCode seam config (opencode-seam-010): generated only when the adopter
  // has no OpenCode config (see findOpenCodeConfig).
...
function isArggonGeneratedConfig(root: string, rel: string): boolean {
  ...
  try {
    return readFileSync(join(root, ...rel.split("/")), "utf8").includes(OPENCODE_CONFIG_SIGNATURE);
  } catch {
    return false;
  }
}
