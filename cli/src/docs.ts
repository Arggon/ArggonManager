 ...
 * value is not provenance. F1 (PR #332 review): that signature comment is
 * only trusted within the file's FIRST TWO lines, which is exactly the
 * generated template's shape (`{` on line 1, signature comment on line 2). A
 * signature-shaped comment after those lines (e.g. below JSON content) is
 * adopter text, not ours, and must not be claimed.
 */
