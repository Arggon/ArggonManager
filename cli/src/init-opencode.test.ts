...
describe("opencode seam: conditional config", () => {
  ...
  // F1 (PR #332 review): the anchor only trusts the signature within the
  // file's first two lines — the generated template is `{` on line 1 with its
  // signature comment on line 2. A config whose ONLY signature mention sits in
  // a comment below JSON content (the first comment line, but outside the
  // window) is an adopter config, not ours.
  ...
  // recognized and silently regenerated rather than present-skipped.
  it("still claims the generated file's line-2 signature comment (F1 anchor)", () => {
    const dir = tempDir();
    runInit({ dir, force: false });
    const lines = readFileSync(join(dir, "opencode.jsonc"), "utf8").split(/\r?\n/, 2);
    expect(lines[1]).toContain("generated by `arggon init`");
