...
describe("opencode seam: conditional config", () => {
  ...
  // F1 (PR #332 review, restored by bug-seam-signature-anchor-regressed): the
  // anchor only trusts the signature within the file's first two lines — the
  // generated template is `{` on line 1 with its signature comment on line 2.
  // A config whose ONLY signature mention sits in a comment below JSON content
  // is an adopter config, not ours.
  it("does not claim a config whose signature comment sits below JSON content (F1)", () => {
    const dir = tempDir();
    const adopterBytes = [
      "{",
      '  "adopter": true,',
      '  // "generated by `arggon init`" is what arggon writes; this file is mine',
      '  "formatter": false',
      "}",
      "",
    ].join("\n");
  ...
  // bug-seam-signature-anchor-regressed literal repro: the signature comment
  // sits on line 2 but BELOW JSON content on line 1, so it is inside the
  // two-line window yet is not the generated shape (`{` on line 1). The merged
  // two-line anchor alone still claimed this file; the opener guard must not.
  it("does not claim the 2-line below-JSON repro: JSON on line 1, signature on line 2", () => {
    const dir = tempDir();
    const adopterBytes = ['{ "formatter": true }', "// generated by `arggon init`", ""].join("\n");
    writeFileSync(join(dir, "opencode.jsonc"), adopterBytes, "utf8");
    expect(findOpenCodeConfig(dir)).toBe("opencode.jsonc");
    const dry = runCli(["init", dir, "--dry-run", "--json"]);
    expect(dry.status).toBe(0);
  ...
  // F1 positive half: the generated shape (line-1 `{`, line-2 signature
  // comment) stays inside the window, so arggon's own file is still
  // recognized and silently regenerated rather than present-skipped — on LF
  // and CRLF alike.
  for (const eol of ["\n", "\r\n"]) {
    it(`still claims the generated line-2 signature comment (${eol === "\n" ? "LF" : "CRLF"})`, () => {
      const dir = tempDir();
      runInit({ dir, force: false });
      const generated = readFileSync(join(dir, "opencode.jsonc"), "utf8");
      const crlf = generated.replace(/\r?\n/g, eol);
      writeFileSync(join(dir, "opencode.jsonc"), crlf, "utf8");
      expect(crlf.split(/\r?\n/, 2)[1]).toContain("generated by `arggon init`");
