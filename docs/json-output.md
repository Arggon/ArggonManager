...
| `linkedNodeModules` | `boolean` | `true` when `--worktree` linked the primary checkout's `node_modules` into a worktree that lacked one (bug-start-worktree-node-modules): the link lets a dependency-needing pre-commit gate (`npm run arggon -- validate`) run in a fresh worktree, is gitignored, and is never committed (the claim commit stages only the item file). `false` when no link happened (no `--worktree`, the primary has no `node_modules`, the worktree already had one, or the best-effort link could not be created). Additive within `schemaVersion: 1` |
