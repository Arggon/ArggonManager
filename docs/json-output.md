...
| `commit`             | `object`   | Tracker auto-commit outcome for the files this run wrote (bug-init-leaves-docs-untracked-start-blocks-on-clean-tree): ONE `{ hash, message }` commit (`chore(tasks): generated init docs (N files)`) covering the generated/updated docs, templates, the tasks/ tree and `tasks/.convention.yml` — or `{ skipped: <reason> }` (`--no-commit`, non-git tree, git absent, nothing to commit). Written paths a `.gitignore` rule matches are never staged (bug-init-ignored-artifacts-dirty-commit: one ignored path aborts `git add` after staging the rest): they are partitioned out and listed in the additive `ignored: string[]` (root-relative posix, sorted), the commit covers the rest, and when every written path is ignored the skip reason is `all mutated paths are ignored by .gitignore`. Additive within `schemaVersion: 1`. |
...
| `dryRun`             | `boolean`  | `true` with `--dry-run` (task-init-dry-run-plan): the payload describes exactly what a real run with the same flags would do; NOTHING is written — no files, no `backup/` dir, no auto-commit, no state mutation (pure read; `commit` is omitted). Additive within `schemaVersion: 1`. |
| `plan`               | `array`    | Only with `--dry-run`. One entry per destination the run would touch or consider: `{ dest, decision, reason, backupDest? }`. `decision` is `created` (missing on disk), `updated` (untouched; regenerated from the current template), `modified-skip` (adopter-modified, kept), `modified-backup` (`--backup`: archive then regenerate; `backupDest` carries the `backup/<date>` path), `acked-skip` (acknowledged baseline, never regenerated), `present-skip` (an adopter OpenCode config exists at one of the four discovered shapes, so the `opencode.jsonc` seam is not generated), `project-name-unrecoverable` (generated content exists but the project name cannot be recovered, so name-bearing writes are skipped instead of baking in the directory basename), `stale` (template no longer generated; orphaned `x-generated` entry), or `overwritten` (scaffold only: `--force` recopy of `tasks/.convention.yml` / `templates/*`). The bucket arrays (`created[]`, `updated[]`, ... and `restored[]`) are computed from the same plan, so a `--dry-run` payload maps 1:1 onto the real run that follows. Additive within `schemaVersion: 1`. |
| `proposals`          | `array`    | Only with `--propose` (task-init-propose-acked-updates; also present with `--dry-run --propose`). Side-file upgrade proposals for acked/adopter-modified destinations whose CURRENT template render differs from disk: `{ dest, proposalPath, decision, template, basedOnVersion, added?, removed?, mode?, regions?, note? }`. `decision` is `proposed` (side file written to `<dest>.proposed-<arggonVersion>`; `added`/`removed` carry the line-diff totals; `mode` is `sections` — the DEFAULT (spec-propose-section-backports-007): only the regions the template ADDED/CHANGED since the dest's first committed version, with `regions` as the per-region `[{ kind: added \| changed, added, removed }]` summaries — or `whole-file` with `--propose-whole-file` / when no git-history baseline is recoverable), `absorbed` (the destination now matches upstream — this run removed its own same-version leftover side file), `stale` (a side file from a DIFFERENT (older) arggon version is still on disk — reported, never deleted), or `informational` (removed-only template drift: nothing to backport, no side file written; `note` explains). Originals stay byte-untouched, `x-generated` state is never mutated, nothing is committed (proposals are untracked working files); `commit` is omitted. Additive within `schemaVersion: 1`. |
...
| `docs.outdated`     | `number`  | Docs whose CURRENT template render differs from the on-disk content (task-doctor-outdated-bucket, additive): the upstream template moved since the doc was generated/acked, so it is not receiving upstream improvements. Orthogonal to the local-state buckets — applies to `untouched`, `modified`, `acknowledged`, and `acknowledgedDrifted` docs alike; docs in `stale` or `missing` are never outdated, and a template file that is missing or unreadable counts as "cannot decide" (never outdated, never a throw). Report-only: doctor re-renders the template exactly as init would (same resolution, `{{PROJECT_NAME}}`/`{{YEAR}}` placeholders, provenance marker) and compares with the on-disk bytes — it never writes. |
...
| `cascadeSkipped` | `Array<{ id, type, reason: "acceptance-incomplete" \| "subtree-open" \| "lock-timeout", sibling? }>` | `update` only: ancestor containers the cascade did NOT complete, with the reason — always present as an array, `[]` when nothing was skipped (bug-cascadeskipped-array-alignment; same always-arrays convention as `autoCompleted`/`cascadeLevels`). `acceptance-incomplete` (task-cascade-acceptance-aware): the container's own body still has unchecked acceptance checkboxes. `subtree-open` (task-cascade-subtree-open-visibility): the walk stopped because a sibling subtree still holds a non-terminal item — `sibling` carries that blocking sibling's id (direct child of the skipped container). `lock-timeout` (bug-cascade-lost-update): the cascade holds each ancestor's item lock across its read-decide-write (acquired strictly child → ancestors upward, deadlock-free); when the ancestor lock cannot be acquired within the lock budget, the walk reports this skip instead of writing unguarded — the child write has already landed, and the next terminal update retriggers the cascade. Additive. |
| `movedFrom` | `string` | `update` only, present when `--parent` moved the item (task-update-reparent) or `--type story` promoted it (task-promote-task-to-story): absolute path of what moved — the item file for a leaf (task/bug), the item directory for a container (story/epic). Additive within `schemaVersion: 1`. |
| `renamedFrom` | `string` | `update --type story` only, present when the promotion renamed the id (task-promote-task-to-story): the OLD id (`task-x`) — a container id must not start with `task-`/`bug-`, so the promoted story becomes `story-x` and `depends_on` references to the old id are rewritten tree-wide. Additive within `schemaVersion: 1`. |
...
Tracker auto-commit (story-tracker-hygiene): `init` (bug-init-leaves-docs-untracked-start-blocks-on-clean-tree), `create`, `update`, `comment`, `adopt`, `cleanup --prune`, and `import-issues` commit ONLY their own mutated paths (`chore(tasks): <verb> <id>`; init uses `chore(tasks): generated init docs (N files)` for its bootstrap commit — generated docs, templates, the tasks/ tree and `.convention.yml` together — so a fresh init never leaves untracked tool-generated state that would block `start`'s clean-tree precondition); the user's pre-existing dirty files are never staged. Mutated paths a `.gitignore` rule matches are skipped (never force-added) and reported in the additive `ignored: string[]` array — one ignored path would otherwise abort `git add` after staging the rest, leaving a dirty index with no commit (bug-init-ignored-artifacts-dirty-commit). `--no-commit` opts out per invocation; `tasks/.convention.yml` `x-tracker.auto-commit: false` opts out tree-wide (flag wins over config, config wins over the default `true`). Skips are reported, never failures — the CLI works without git. Under git `index.lock` contention (bug-autocommit-silent-skip) the commit is retried with a short bounded backoff; if a skip is still unavoidable it is reported as `{ skipped: "git index locked" }` AND as a stderr warning (`commit skipped: git index locked`) — never silent. A "nothing to commit" result is a quiet benign skip ONLY when the mutated paths are genuinely clean (someone else committed the same content); if the paths still carry changes, a concurrent index rewrite lost this process's staged entry (task-nothing-to-commit-masking) and the skip is reported instead as `{ skipped: "nothing to commit (staged entry lost under contention)" }` plus a stderr warning — the mutation stays written-but-uncommitted for the caller to retry. Additive within `schemaVersion: 1`.
...
| `handoff.session`       | `string`   | Present when `--session` resolves to a non-empty value or the client sent a `_meta.sessionID` with a non-empty normalized value; provenance session identifier rendered in the heading; capped at 64 chars |
...
| `linkedNodeModules` | `boolean` | `true` when `--worktree` linked the primary checkout's `node_modules` into a worktree that lacked one (bug-start-worktree-node-modules): the link lets a dependency-needing pre-commit gate (`npm run arggon -- validate`) run in a fresh worktree. The link is untracked — a `node_modules/` ignore pattern matches directories only, so it is NOT ignored — but start never commits it: the claim commit stages only the item file. Before a configured `x-worktree.post-start` hook runs, start removes the link (so the canonical `npm ci` cannot reify through it and empty the primary install) and re-creates it only when the hook leaves the worktree without `node_modules`; the field still reports that start linked this run. `false` when no link was created (no `--worktree`, the primary has no `node_modules`, the worktree already had one, or the best-effort link could not be created). Additive within `schemaVersion: 1` |
...
With `--worktree` the claim commit, push, and draft PR run inside the worktree and the item's `worktree_path` is persisted (re-runs attach to the recorded path). Start prepares the worktree before the claim commit (the primary checkout's `node_modules` is linked in when the worktree lacks one) and never rolls the worktree back on failure: a failure after the worktree exists keeps the worktree and branch, and the failure message (human output and `error.message`) names the failing step, the kept path, a step-specific remediation and the fact that re-running `arggon start <id> --worktree` attaches to it (a failed push is the exception: attach does not retry it, and the message instructs `git push -u origin <branch>` manually) — the pre-commit gate is never bypassed (`--no-verify` is never passed). When the item carries an imported GitHub issue number (`WorkItem.issue`), the PR body ends with `Closes #N` so GitHub closes the issue when the PR merges; items without it are unchanged. Failures use `error.code: "START_FAILED"` (unknown id, taken claim — never forced, dirty tree, existing branch, non-git tree, worktree path occupied by a non-worktree, a failing project gate/hook, or gh failure).
...
```json
{
  "budget": {
    ...
    "mcp": {
      ...
      "tokenEstimate": 2260,
      "largestTools": [
        { "name": "arggon_update", "bytes": 1824 },
        { "name": "arggon_list", "bytes": 1417 },
        { "name": "arggon_handoff", "bytes": 1281 }
      ]
    }
  }
}
...
The additive `mcp` object (task-schema-budget) measures the LIVE MCP `tools/list` payload, fetched in-process against `runMcpServer` (the same surface `mcp-parity.test.ts` drives): `totalBytes` is checked against the advisory `MCP_TOOLS_BUDGET_BYTES` (12,288 B — ~36% headroom over the 9,040 B live baseline recorded 2026-09-15, commit `5d6c504`), `tokenEstimate` is ~chars/4 (the baseline method), and `largestTools` names the three heaviest tool definitions so schema bloat is attributable, not just visible.
