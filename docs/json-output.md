...
| `plan`               | `array`    | Only with `--dry-run`. One entry per destination the run would touch or consider: `{ dest, decision, reason, backupDest? }`. `decision` is `created` (missing on disk), `updated` (untouched; regenerated from the current template), `modified-skip` (adopter-modified, kept), `modified-backup` (`--backup`: archive then regenerate; `backupDest` carries the `backup/<date>` path), `acked-skip` (acknowledged baseline, never regenerated), `stale` (template no longer generated; orphaned `x-generated` entry), or `overwritten` (scaffold only: `--force` recopy of `tasks/.convention.yml` / `templates/*`). The bucket arrays (`created[]`, `updated[]`, ... and `restored[]`) are computed from the same plan, so a `--dry-run` payload maps 1:1 onto the real run that follows. Additive within `schemaVersion: 1`. |
