...
Each `docs` entry falls in exactly one bucket (`missing`, else `stale`, else `acknowledged`, else `untouched`/`modified`), so `untouched + modified + acknowledged + stale + missing = managed`. Failures use `error.code: "DOCTOR_FAILED"` (unexpected errors only — a missing tree is a normal report).
...
| `cascadeSkipped` | `Array<{ id, type, reason: "acceptance-incomplete" }>` | `update` only, present when non-empty: ancestor containers the cascade did NOT complete because their own body still has unchecked acceptance checkboxes (`task-cascade-acceptance-aware`). Additive. |
...
| `postStart` | `object` | Outcome of the `x-worktree.post-start` hook (task-start-post-hook): `{ command, ok: true }` or `{ command, ok: false, error }` (`error` = `post-start failed: <command> → <stderr tail>`; hook failure never fails the start). Present only when a new worktree was created and a hook ran (no config, `--no-hook`, or attach: field absent). Additive within `schemaVersion: 1` |
...
| `pruned`     | `object[]`   | `{ id, action }` entries performed (empty without `--prune`); a failed step reports `action: "failed"` with additive `error` and, when the branch delete failed after the worktree was removed, `leftoverBranch: <name>` — per-candidate, the run continues |
