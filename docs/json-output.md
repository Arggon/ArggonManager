...
The additive `mcp` object (task-schema-budget) measures the LIVE MCP `tools/list` payload, fetched in-process against `runMcpServer` (the same surface `mcp-parity.test.ts` drives): `totalBytes` is checked against the advisory `MCP_TOOLS_BUDGET_BYTES` (12,288 B — ~35% headroom over the 9,050 B live baseline recorded 2026-09-15), `tokenEstimate` is ~chars/4 (the baseline method), and `largestTools` names the three heaviest tool definitions so schema bloat is attributable, not just visible.
