...
Derived from `docs/specs/spec-opencode2-009.md`. Each task carries a
...
- Write `docs/specs/spec-opencode-seam-NNN.md` + plan: the exact generation
...
- **Acceptance:** report-only (exit 0), bounded output; tests for V1-shaped and
  clean fixtures; `docs/json-output.md` documents the block.

## T5: W1d — Playbook and reference docs

- `docs/playbooks/opencode.md` (2.0.7) through the playbook pipeline;
  `docs/agents.md` §Reference integrations gets the OpenCode V2 subsection;
...
- Generated `arggon-coordinator` (primary), `arggon-worker`, `arggon-reviewer`
  (edit-deny) with system prompts derived from `docs/agents.md` orchestration;
