...
| --- | --- | --- |
| **Claim exclusivity** | Run N concurrent `arggon start <id>` processes: same assignee, and different assignees. | Single winner per the file lock (`docs/claim.md`): same assignee → exactly one `created: true`, others attach; different assignees → exactly one wins, others `START_FAILED` claim conflict. No last-write-wins. (bug-claim-race-no-lock) |
...
1. Grep the promise sources for normative markers: `SKILL.md`
   (`.agents/skills/arggon-cli/SKILL.md`), `docs/agents.md`,
   `docs/convention.md`, `docs/claim.md` — patterns: `never`, `always`,
...
docs/agents.md §Adoption step 6) promises "Hand edits made AFTER this ack still
