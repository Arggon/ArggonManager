...
2. **Tracker commit history of the live experiment repos**:
   ...
   — `git log --oneline -- tasks/` for tracker-mutation patterns.
...
- **Manual tracker git work**: a manual `git add tasks/ && git commit` right
  after tracker mutations. Either pre-auto-commit leftover behavior or a
  mutation the auto-commit does not cover.
- **Resets/reverts of tracker files**: `git revert`/`reset`/`checkout -- tasks/…`
...
2. **Dedupe**: against already-filed items (`grep -r "<keyword>" tasks/`) and
...
Same as the audit protocol (`docs/labs/adversarial-audit.md` §Filing rules):
