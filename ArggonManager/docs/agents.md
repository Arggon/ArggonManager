...
V2 recognizes `AGENTS.md` only — **no `CLAUDE.md` fallback** (that shim serves other tools) — and accepts but does not load the `instructions` config array. Never map V1 fields into V2 config (`mcp.<name>`, `enabled`, `autoupdate`); the field-level source of truth is `https://opencode.ai/config.json`. `arggon init` also bundles the optional OpenCode V2 plugin (`.opencode/plugins/arggon/index.ts`, same never-overwrite + provenance semantics as the skills): auto-discovered with zero config, it registers the `arggon` MCP server (`{type:"local",command:["arggon","mcp"]}`) **only when no `arggon` server is already configured**, never clobbers one, and is failure-isolated — a plugin error logs once and no-ops, never breaking a session, the CLI or the MCP server. On top of MCP auto-registration the plugin correlates the session to the active work item — `ARGON_ITEM` env → observed `arggon` calls → `feat/<id>`/`fix/<id>` branch — injects a **bounded** item block through the context hook, rebuilt per call so it survives compaction, renames the session on claim, and logs a non-blocking hygiene warning when `arggon validate` fails after a commit; ADR 0011's native-first rebuild (`plan-native-first-011`) adds the **native `arggon` tool namespace** (W2, `task-native-tools`): twelve Code Mode tools registered with `ctx.tool.transform` (`options.namespace: "arggon"`, `codemode: true`) that call the kernel in-process through `@arggon/lib` and return the documented `--json` envelopes, with kernel failures surfacing as typed tool errors and the session continuing; the kernel import is guarded, so a dependency-less tree registers no tools until the W3 bundle lands. Headless evidence: `npm run smoke:opencode` — 12 scenarios on opencode v2.0.10). Config precedence, skills discovery, testing and the upgrade policy live in the [OpenCode playbook](./playbooks/opencode.md) (pinned 2.0.10).
