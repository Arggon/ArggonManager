...
**Decisión para este spec:** el matching se hace por id del item contenido en el headRefName. El pattern esperado es `<type>/<id>[-<slug>]` o similar. Ver la convention de nombres de branch en ArggonManager/docs/convention.md. Si no hay headRefName que contenga el id del item, no hay match.
