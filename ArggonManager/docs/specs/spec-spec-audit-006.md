...
Invariant: **report-only** — the command reads `ArggonManager/docs/specs/*.md` and writes
...
`ArggonManager/docs/specs/` (top level only, matching `spec validate`/`spec analyze` scope).
...
**Failure policy**: failures are structural (missing `ArggonManager/docs/specs` dir, empty
  ...
  "findings": [
    {
      ...
      "files": ["ArggonManager/docs/specs/a.md", "ArggonManager/docs/specs/b.md"],
      "similarity": 0.87,
...
- [ ] `--json` envelope matches the contract above; missing/empty `ArggonManager/docs/specs` fails cleanly (SPEC_FAILED, exit 1)
