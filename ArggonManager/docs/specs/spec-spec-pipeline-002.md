...
Feature specs (`ArggonManager/docs/specs/spec-<slug>-NNN.md`) and implementation plans
(`ArggonManager/docs/plans/plan-<slug>-NNN.md`) are written by agents, so agents must be able
...
- `validate` checks `ArggonManager/docs/specs/*.md` and `ArggonManager/docs/plans/*.md` under the repo root
...
- Issues are `{ path, message, code }` like `validate`; codes include
  ...
  `PLAN_DUPLICATE_ID`.
- `spec new` numbers globally: max existing NNN across `ArggonManager/docs/specs` + `ArggonManager/docs/plans`
