...
### 2026-09-20 @Arggon
### 2026-09-20 @Arggon — revisión final (head `2952a88`, PR #376; CI `cli` pass con el step nuevo)

**F1 verificado — fix correcto.** Reproduje la mutación en un worktree scratch de `2952a88` (sin tocar el branch): acorté una descripción en `opencode/plugins/arggon/index.ts` sin rebuild → `cli/src/plugin-copy.test.ts` **falla** (1 failed | 3 passed) con el mensaje accionable `opencode/plugins/arggon/index.bundle.ts is stale — run `npm run build:plugin` and commit the artifact`; `npm run check:plugin` **exit 1** mostrando el diff del artefacto. Revertido: tests 7/7 verdes y `check:plugin` exit 0. El auto-heal quedó solo para la copia derivada gitignoreada `.opencode/plugins/arggon/index.ts` (la borré y el test la regeneró); `opencode/plugins/arggon/bundle.test.ts` es read-only (solo `mkdtempSync`/`copyFileSync` a temp/`rmSync`). El step "Fail if the committed plugin bundle drifted from its source" corre **después** de `npm run build` en el workflow `cli` y figura `success` en el run 35520799679 (job 106104417417).

**Menores cerrados.** `/arggon-status` usa el input nativo (`{ status: "blocked" }` / `{ stale: true }`, con el DSL `filter:` mencionado como alternativa); docstring de `typecheck.test.ts` actualizado (sin `@opencode/plugin`, sin "init vendors the source"); `init-opencode.test.ts` valida el frontmatter de los 11 comandos (description, agent ∈ {coordinator, reviewer}, `subagent: true` en review) y el header del smoke documenta que corre bodies, no el loader V2.

**Gates reproducidos en `2952a88`** (todos verdes): `npm test` 86 archivos / **1394 tests**; lint limpio; `npm run build` ok; `npm run check:plugin` exit 0; `arggon validate` ok (0 warnings); `arggon spec validate` ok; `smoke:opencode` **23 escenarios / 0 fallos (104 checks), exit 0** (incluye /status con el body nuevo y correlación desde `tools.arggon.show`); `context:report --strict` todos los bounds (AGENTS 1.959 ≤ 2.048; native 12/8 pinned 11.097 ≤ 12.288; item block 252 ≤ 1.024; fijo 24.505). Artefacto del bundle byte-idéntico al revisado (`92fc312c…`, 282.381 B). Sin cambios en kernel/CLI/MCP: el diff solo toca plugin/templates/tests/smoke/docs/CI/package.json.
