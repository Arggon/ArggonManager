...
**Convention (v0) is locked.** See [`docs/convention.md`](ArggonManager/docs/convention.md) for folder layout, frontmatter schema, statuses, and claim rules. A sample tree lives in the fixture [`fixtures/tasks-valid/tasks/launch-mvp/`](fixtures/tasks-valid/tasks/launch-mvp/) (plus its tracker `.convention.yml`). Copy-paste templates for each type live in [`templates/`](templates/).
...
Exact v0 fields are documented in [`docs/convention.md`](ArggonManager/docs/convention.md) (layout, schema, and status transitions are locked). Omit `assignee` when unassigned (do not write an empty `assignee:`).
...
- [OpenCode V2 playbook](ArggonManager/docs/playbooks/opencode.md) — V2 setup, conventions, testing and upgrade policy (pinned 2.0.8)
...
Copy a stub into the tracker root (`ArggonManager/`) per [`docs/convention.md`](ArggonManager/docs/convention.md), or use `arggon create` (copies from these templates). Spec/plan stubs are rendered into `docs/specs/` / `docs/plans/` by `arggon spec new` (see below).
...
- `initialized`: whether `ArggonManager/.convention.yml` was found, plus the convention version (0-3).
...
`--dry-run` prints the same inventory (managed vs adopter-owned vs absent, per-doc sizes, stack hints) and the planned story/task creation without writing anything — including the default story. Executing agents follow the checklist in the task body; the full procedure is documented in [docs/agents.md](ArggonManager/docs/agents.md) §Adoption sweep.
...
- `--json`: one JSON object on stdout; failures emit `code: "BOARD_FAILED"`. The payload contract for every mode (which fields appear with `--serve`, `--github`, `--group-by`) is documented once in [docs/json-output.md](ArggonManager/docs/json-output.md) — the `board` section there is the normative source
...
One-shot migration of an existing GitHub issue backlog into the tracker root (`ArggonManager/`) (the docs/agents.md §0 promise). Reads issues via `gh issue list --state all --limit 200 --json number,title,state,body,labels` and writes one task per issue through the same kernel as `create`/`update`. **Idempotent**: target ids are `task-issue-<number>`, so a re-run imports nothing (`created: 0`, everything skipped).
