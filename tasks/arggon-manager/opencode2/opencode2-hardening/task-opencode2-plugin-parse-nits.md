...
### 2026-09-18 @Arggon
Claim correction (F3 of task-opencode2-plugin-quote-spans): the 2026-09-18 decision "nothing left as a live FP" was too broad. Two pre-existing live false positives remained at merge time and are fixed under task-opencode2-plugin-quote-spans: (1) `splitTokens` split on whitespace inside quotes, so `"arggon show task-x"`, `command "arggon show task-x"`, `npm run "arggon show task-x"` and `x="line1\narggon show task-x"` correlated non-executing mentions (bash: 127 / missing script / assignment only), while `x="a b" arggon show task-x` was a miss (bash runs arggon); (2) escaped `\(` tokens in command position were treated as group openers (`\(arggon show task-x)`, `X=1 \(arggon show task-x)`, both bash syntax errors). No reopening: this item stays done; scope moved to the follow-up item.
