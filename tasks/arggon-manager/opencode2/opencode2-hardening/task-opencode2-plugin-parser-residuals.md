...
**Gates.** focused `opencode/plugins/arggon/index.test.ts` 42/42; full suite 75 files / 1270 tests green (private TMPDIR); `npm run lint` green; `npm run build` green; `arggon validate` ok 0 warnings; `arggon spec validate` ok (16 docs, 0 warnings); `npm run smoke:opencode` 11 scenarios, 0 failures.
