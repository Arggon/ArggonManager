...
- [x] `docs/opencode2.md` gains a "Side-by-side installs" section documenting
      both recipes, with the exact commands verified here.
- [x] It states the `mise.toml` (tracked, inherited by worktrees and clones)
      vs `mise.local.toml` (machine-local, worktrees do not inherit it)
      trade-off and recommends the tracked file for oc2 projects.
- [x] It explains why bare `arggon` must resolve per project (MCP command,
      skills, plugin) and the launch-dir/server-env caveat.
- [x] Verification steps included: `which arggon`, `Ctrl+P → Plugins`,
      `opencode api plugin.list`.
- [x] The tsc exec-bit gotcha and the generated-copies bootstrap are stated in
      the dev setup path.
- [x] Docs only — no repo code changes required.
...
- Option A is already mounted locally (wrapper + `arggon-oc2` +
  ...
  reproduce it.
- Reassessment context: the CLI side-by-side story is under review by the
  `opencode2-native` program (`task-native-capability-audit`); if the redesign
  drops the CLI, this doc is rewritten with the native install story.
