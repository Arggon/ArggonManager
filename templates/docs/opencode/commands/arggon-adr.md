...
The process and the minimum template live in `ArggonManager/docs/engineering.md` §ADR process
— read it first and follow it; do not invent a new format or location.
...
1. Confirm the decision qualifies (stack/package layout, identity model,
   ...
   do not need an ADR.
2. Number it 4-digit monotonic under `ArggonManager/docs/adr/NNNN-short-title.md` and keep the
   template sections: Status / Date / Deciders, Context, Decision,
   Consequences, Alternatives considered.
3. Ground it in the exploration or spec that produced it (`ArggonManager/docs/explorations/`)
