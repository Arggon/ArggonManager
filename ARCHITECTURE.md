...
tasks, bugs) are Markdown files under `tasks/`, managed by the `arggon` CLI, reviewed and
...
- **Agent-first contract**: every command has a `--json` mode emitting a stable
  `schemaVersion: 1` envelope ([docs/json-output.md](docs/json-output.md)); agents and
...
([tasks/.convention.yml](tasks/.convention.yml)), commit tracker mutations via
...
provenance into the `x-generated` section of `tasks/.convention.yml` so re-runs are
...
ArggonManager/
  cli/src/          # the CLI: one module per command + shared kernel, tests co-located
    ...
    convention.ts|paths.ts|detect-repo.ts
                    # tasks/.convention.yml parsing, repo/worktree resolution
  docs/             # process & product documentation (humans and agents)
    ...
    specs/ plans/   # feature specs and implementation plans
    playbooks/      # technology playbooks (arggon playbook)
  tasks/            # DATA, not code: the git-native work tree this repo dogfoods
...
- [docs/convention.md](docs/convention.md) is the source of truth for schema/layout/
...
- All tree mutations go through command modules that run validation gates; raw
  `tasks/**` edits that bypass validation are caught by CI and the pre-commit hook.
- `tasks/` is data: product code must not depend on its contents (fixtures and tests
...
- The JSON contract (`schemaVersion: 1`) is additive-only; envelope fields are documented
  in [docs/json-output.md](docs/json-output.md) in the same PR that changes them.
- Stack decisions (TypeScript/ESM, commander, vitest) are ADR-recorded; changes need a
  new ADR ([docs/adr/](docs/adr/)).
...
- One branch per item, derived from `branch_patterns` in `tasks/.convention.yml` →
